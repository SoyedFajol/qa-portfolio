...
export const TOPICS = [
  ...
  { id: 'security', icon: '🔐', label: 'Security Testing' },
  { id: 'agile', icon: '🌀', label: 'Agile / Scrum' },
...
export function sampleFallbackQuiz(topicId, count = 15) {
  const bank = FALLBACK_QUIZZES[topicId]
  if (!bank) return null
  const pool = [...bank.questions]
  // freeze the target BEFORE the loop — pool.length shrinks as we splice,
  // and re-reading it live silently truncated sessions (caught in prod)
...
export const FALLBACK_QUIZZES = {
  ...
  security: {
    lesson:
      'Security testing means thinking like an attacker before one shows up. The core map is the OWASP Top 10: injection (user input reaching interpreters), broken authentication and access control, XSS, and misconfiguration. Golden rules: never trust client-side input — validate on the server; authenticate (who are you?) before you authorize (what may you do?); encrypt in transit with HTTPS; store passwords only as strong hashes; keep secrets out of code. As a tester you probe with hostile inputs, borrowed IDs, and repeated requests — every finding you report is an incident that never happens.',
    questions: [
      {
        question: 'Typing <script>alert(1)</script> into a comment field and seeing it execute on the page proves…',
        options: ['A performance bug', 'Cross-Site Scripting (XSS) — user input rendered as live code', 'A database crash', 'A CSS bug'],
        correctIndex: 1,
        explanation: 'XSS happens when input is rendered without escaping, letting attackers run scripts in other users’ browsers — steal sessions, fake UI, worse.',
      },
      {
        question: "Entering ' OR '1'='1 into a login field tests for…",
        options: ['Boundary values', 'SQL injection — input breaking out of the query into the interpreter', 'Slow queries', 'Password strength'],
        correctIndex: 1,
        explanation: 'If raw input is glued into SQL, crafted quotes rewrite the query logic. Parameterized queries are the fix; QA probes with quote payloads.',
      },
      {
        ...
        correctIndex: 0,
        explanation: 'Login proves identity; permissions decide access. Bugs in the SECOND one (doing things you shouldn’t) are the nastier class.',
      },
      {
        question: 'You change /orders/1041 to /orders/1042 and see another customer’s order. This is…',
        options: ['Expected behavior', 'IDOR — Insecure Direct Object Reference (broken access control)', 'A caching bug', 'A UI typo'],
        correctIndex: 1,
        explanation: 'The server trusted the ID without checking ownership. One of the most common and most severe real-world findings — always test with someone else’s IDs.',
      },
      {
        question: 'Passwords in the database should be stored as…',
        options: ['Plain text for support convenience', 'Encrypted with a key kept nearby', 'Strong one-way hashes (bcrypt/argon2) with salt', 'Base64'],
        correctIndex: 2,
        explanation: 'Hashing is one-way: even a stolen database doesn’t yield passwords. Base64 is encoding, not protection — a classic interview trap.',
      },
      {
        question: 'HTTPS protects against…',
        options: ['All hacking', 'Reading or tampering with data in transit between browser and server', 'SQL injection', 'Weak passwords'],
        correctIndex: 1,
        explanation: 'TLS encrypts the channel. It does nothing about flaws in the application itself — a perfectly encrypted connection can still deliver an XSS payload.',
      },
      {
        question: 'A login form allows unlimited attempts. The risk is…',
        options: ['None', 'Brute-force/credential-stuffing — automated guessing until something works', 'Slow page loads', 'Cookie overflow'],
        correctIndex: 1,
        explanation: 'Rate limiting, lockouts and CAPTCHAs exist to make guessing expensive. QA verifies them with scripted bursts — politely, on systems you’re authorized to test.',
      },
      {
        question: 'CSRF (Cross-Site Request Forgery) tricks a user’s browser into…',
        options: ['Downloading malware', 'Sending authenticated requests the user never intended (e.g., a hidden money transfer)', 'Displaying wrong fonts', 'Clearing cache'],
        correctIndex: 1,
        explanation: 'Because browsers auto-attach cookies, a hostile page can fire requests as YOU. Anti-CSRF tokens and SameSite cookies are the defense to test for.',
      },
      {
        question: 'Which header set improves a web app’s security posture?',
        options: ['X-Powered-By and Server', 'Content-Security-Policy, X-Content-Type-Options, Referrer-Policy', 'Accept-Language', 'Cache-Control only'],
        correctIndex: 1,
        explanation: 'CSP limits where scripts load from, nosniff stops MIME confusion, Referrer-Policy limits leakage. A headers scan is a five-minute test with real value.',
      },
      {
        question: 'An API key committed to a public GitHub repo is…',
        options: ['Fine if the repo is small', 'Compromised the moment it lands — rotate it and scrub the history', 'Safe if renamed', 'Only a style issue'],
        correctIndex: 1,
        explanation: 'Bots scan public commits within seconds. Secrets belong in environment variables; leaked ones get rotated immediately, not just deleted.',
      },
      {
        question: 'The principle of least privilege says…',
        options: ['Everyone gets admin to move fast', 'Each account/service gets only the access it needs to do its job', 'Privileges never change', 'Only managers get access'],
        correctIndex: 1,
        explanation: 'Smaller blast radius: when (not if) something is compromised, it can do less damage. Test by trying admin actions from ordinary accounts.',
      },
      {
        question: 'A stack trace with file paths appears on a production error page. Why report it?',
        options: ['It looks unprofessional only', 'Information disclosure — internals help attackers aim their next probe', 'It slows the server', 'No need to report'],
        correctIndex: 1,
        explanation: 'Frameworks, paths and versions map the attack surface. Production errors should show a safe generic message while logging details server-side.',
      },
      {
        question: 'Your npm audit shows a critical vulnerability in a dependency. The right move is…',
        options: ['Ignore it — you didn’t write that code', 'Assess exposure, update/patch it, and add the audit to CI so the next one is caught automatically', 'Delete node_modules', 'Switch languages'],
        correctIndex: 1,
        explanation: 'Most real-world breaches ride on known, unpatched dependencies. Supply-chain hygiene is squarely part of modern QA.',
      },
      {
        question: 'You find a serious vulnerability in a system you were NOT authorized to test. You should…',
        options: ['Exploit it to prove impact', 'Tweet it', 'Stop, and report it responsibly through the owner’s disclosure channel', 'Sell it'],
        correctIndex: 2,
        explanation: 'Responsible disclosure protects users and you. Testing without authorization is illegal in most places — permission first, always.',
      },
      {
        question: 'Security testing differs from functional testing mainly because…',
        options: ['It needs no test cases', 'You verify what the system must NOT allow, not just what it should do', 'It is always automated', 'Only pentesters can do it'],
        correctIndex: 1,
        explanation: 'Functional tests confirm intended behavior; security tests hunt unintended abilities. Every input field and ID parameter is an invitation to ask "what shouldn’t this let me do?"',
      },
    ],
  },
  ...
  agile: {
    lesson:
      'Scrum in one breath: a small cross-functional team delivers a usable product increment every sprint (usually two weeks). The Product Owner orders the backlog by value; the Scrum Master clears obstacles and protects the process; developers — including QA — build and test the increment together. The rituals each have one job: planning commits the sprint, the daily surfaces blockers, the review shows working software to stakeholders, the retrospective improves how the team works. Quality is a whole-team responsibility: acceptance criteria written before code, testing inside the sprint, and a Definition of Done that means DONE.',
    questions: [
      {
        question: 'The three Scrum roles are…',
        options: ['Manager, developer, tester', 'Product Owner, Scrum Master, Developers', 'CEO, PM, QA', 'Analyst, architect, coder'],
        correctIndex: 1,
        explanation: 'The PO owns WHAT (value-ordered backlog), the Scrum Master owns the process, and Developers — a cross-functional group that includes QA — own HOW.',
      },
      {
        question: 'The daily standup exists to…',
        options: ['Report hours to management', 'Sync the team and surface blockers — a planning huddle, not a status ceremony', 'Assign blame for delays', 'Demo features'],
        correctIndex: 1,
        explanation: 'Fifteen minutes for the team, not the boss: what moved, what’s next, what’s stuck. Blockers raised here die young.',
      },
      {
        question: 'A sprint retrospective is about…',
        options: ['The product’s features', 'How the TEAM works — what to keep, drop and try next sprint', 'Salary reviews', 'Writing test cases'],
        correctIndex: 1,
        explanation: 'Review inspects the product; retro inspects the process. A team that skips retros repeats its mistakes on schedule.',
      },
      {
        question: 'Story points measure…',
        options: ['Exact hours', 'Relative effort/complexity of work items', 'Lines of code', 'Team member performance'],
        correctIndex: 1,
        explanation: 'Points compare stories to each other ("this is twice that"). Converting them to hours or performance reviews is the classic anti-pattern.',
      },
      {
        question: 'A good acceptance criterion is…',
        options: ['"Works properly"', 'Specific and testable: "Given an expired card, payment is rejected with error E-102"', 'Written after release', 'Optional for small stories'],
        correctIndex: 1,
        explanation: 'If QA can’t turn it into a pass/fail check, it isn’t a criterion — it’s a wish. Great QA engineers sharpen these in refinement, before code exists.',
      },
      {
        question: 'The Definition of Done exists so that…',
        options: ['Developers can skip testing', '"done" means the same thing for every story: coded, tested, reviewed, integrated', 'Sprints can be extended', 'Managers can track hours'],
        correctIndex: 1,
        explanation: 'Without a shared DoD, "done" quietly becomes "coded". A story that isn’t tested isn’t done — that sentence is QA’s shield.',
      },
      {
        question: 'Velocity is…',
        options: ['How fast developers type', 'The average points a team completes per sprint — a planning aid, not a KPI', 'Story priority', 'Bug count'],
        correctIndex: 1,
        explanation: 'Velocity helps forecast capacity. Weaponizing it as a performance target reliably produces inflated estimates and rushed quality.',
      },
      {
        question: 'In Kanban, WIP limits exist to…',
        options: ['Slow the team down', 'Force finishing over starting — less multitasking, faster flow, bottlenecks exposed', 'Limit team size', 'Cap story points'],
        correctIndex: 1,
        explanation: 'Ten half-done tasks deliver nothing. Capping work-in-progress makes the team swarm on finishing — and makes stuck columns visible.',
      },
      {
        question: 'Sprint review vs retrospective…',
        options: [
          'Review: show the increment to stakeholders. Retro: improve the team’s process.',
          'Both are demos',
          'Review is private, retro is public',
          'They are the same meeting',
        ],
        correctIndex: 0,
        explanation: 'Two different feedback loops: one on the product (with stakeholders), one on the way of working (team only).',
      },
      {
        question: 'Mid-sprint, a stakeholder demands a new feature immediately. The scrum-healthy path is…',
        options: ['Squeeze it in quietly', 'Route it to the Product Owner to prioritize — next sprint, or a formal scope trade if truly urgent', 'Refuse forever', 'Cancel the sprint'],
        correctIndex: 1,
        explanation: 'The sprint commitment protects focus. The PO owns priority calls and trade-offs — side-door requests are how quality and trust leak.',
      },
      {
        question: 'Where does testing happen in an agile sprint?',
        options: ['In a separate testing sprint afterwards', 'Inside the sprint, alongside development — stories are tested before they count as done', 'Only before release', 'Only in production'],
        correctIndex: 1,
        explanation: '"Hardening sprints" are waterfall wearing a costume. Test within the sprint or the DoD is fiction.',
      },
      {
        question: 'A burndown chart flat-lining mid-sprint most likely means…',
        options: ['The team is resting', 'Work is stuck or unplanned work invaded — time to inspect at the daily', 'The sprint is going perfectly', 'Velocity doubled'],
        correctIndex: 1,
        explanation: 'Burndown is a smoke alarm, not a whip: a flat line says stories are blocked, too big, or the sprint got hijacked.',
      },
      {
        question: 'The product increment must be…',
        options: ['A slide deck', 'Usable, working software meeting the DoD — every single sprint', 'A design mockup', 'A promise'],
        correctIndex: 1,
        explanation: 'Agile’s core bargain: value ships in small, DONE slices. If nothing usable exists at sprint end, ceremonies were theater.',
      },
      {
        question: 'Agile beats waterfall for most software because…',
        options: ['No documentation is needed', 'Feedback arrives every couple of weeks, so wrong turns are cheap to correct', 'It has fewer meetings', 'Testing is optional'],
        correctIndex: 1,
        explanation: 'Waterfall bets everything on upfront guesses; agile buys information early and often. It trades certainty theater for course-correction.',
      },
      {
        question: 'QA’s strongest position in an agile team is…',
        options: ['Gatekeeper at the very end', 'Embedded from refinement onward — sharpening criteria, testing continuously, coaching quality', 'Outsourced entirely', 'Optional attendee'],
        correctIndex: 1,
        explanation: 'Quality is designed in, not inspected in at the exit. This is exactly how an SQA engineer works inside Scrum teams at BRAC IT.',
      },
    ],
  },
